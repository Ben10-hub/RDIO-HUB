...
export function ProgramCard({ program, index, featured = false }: ProgramCardProps) {
  return (
    <motion.div
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  <a
                    ...
                    className={program.status === 'closed' ? 'pointer-events-none' : ''}
                  >
                    {program.status === 'closed' ? 'Closed' : 'Apply Now'}
                    {program.status !== 'closed' && <ExternalLink size={16} />}
                  </a>
                </Button>
              </div>
            </div>
               ...
               <Button
                ...
                >
                  <a
                    ...
                    className={program.status === 'closed' ? 'pointer-events-none' : ''}
                  >
                    {program.status === 'closed' ? 'REGISTRATION CLOSED' : 'APPLY NOW'}
                    {program.status !== 'closed' && <ExternalLink size={20} />}
                  </a>
                </Button>
            </div>
          </div>
          ...
          <div className="p-8 md:p-12 flex flex-col gap-12 bg-white">
            ...
            <div>
              <div className="flex items-center justify-between mb-8 border-b-2 border-navy pb-4">
                <h4 className="text-3xl font-extrabold text-navy uppercase tracking-widest">Course Structure & Curriculum</h4>
              </div>
              ...
              <div className="w-full bg-soft-bg border-2 border-border p-3 md:p-4">
                <div className="w-full bg-white border border-border flex flex-col items-center justify-center p-16 md:p-32 text-center aspect-square md:aspect-[21/9] relative overflow-hidden group/img cursor-pointer">
                  ...
                  <div className="relative z-10 flex flex-col items-center transform transition-transform duration-500 group-hover/img:scale-105">
                    ...
                    <h5 className="font-extrabold text-navy text-4xl mb-4 tracking-wide">Detailed Curriculum Roadmap</h5>
