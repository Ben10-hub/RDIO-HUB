...
export function Navbar() {
  ...
  return (
    ...
    >
      {/* Scrolling Banner */}
      <div className={cn(
        "hidden lg:block bg-[#0a192f] border-b border-white/5 overflow-hidden transition-all duration-300",
        ...
        >
          {[...Array(10)].map((_, i) => (
            <span key={i} className="text-gold font-semibold text-xs tracking-[0.2em] uppercase mx-8">
              AIML • GENAI • AGENTIC AI | GOOGLE INTERNSHIP PROGRAM | APPLY NOW
            </span>
          ))}
        </motion.div>
      </div>
      ...
      {/* Top Utility Bar */}
      <div className={cn(
        "hidden lg:block bg-[#0a192f] text-gray-300 text-xs py-2 transition-all duration-300 border-b border-white/10",
      ...
      <nav className="container mx-auto px-4 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <Link href="/" className="flex items-center gap-3">
            <div className="relative w-10 h-10 lg:w-12 lg:h-12 rounded-full overflow-hidden border-2 border-transparent hover:border-gold transition-colors">
              <Image
                src="/images/logo.png"
                alt="RDIO AI"
                fill
